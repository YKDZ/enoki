...
- REST API
- 主页的卡片瀑布流 / 表格视图
- Hub 侧可协调探针卸载；本机也可执行 `sudo enoki-probe uninstall`。管理员可对签名发布转换明确允许的相邻版本发起兼容升级；修复尚未启用，相关请求会稳定报告不支持
- 探针常态下不拥有 root 权限
...
- 管理界面可查看主机元数据和操作状态；当前支持显式兼容升级、Hub 组合卸载和本机卸载，修复尚未启用
...
- 安装器会把 Hub 地址和已验证安装回执写入 root 所有的安装元数据；常驻探针只协调卸载请求，独立的有界生命周期角色验证授权并执行固定清理事务。本地卸载不删除 Hub 中的主机记录
