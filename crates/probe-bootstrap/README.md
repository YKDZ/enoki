...
`enoki-probe-bootstrap` 是 Probe 首次安装的最小信任入口。它不单独分发：每个目标平台只有一个签名的 Probe Asset Bundle，包内固定包含 `probe`、`observation-runtime`、`system-state-provider`、`bootstrap-acquirer` 与 `bootstrap-activator` 五个同版本角色；每个运行角色同时绑定固定权限配置与资源合同。为兼容既有 schema 3 安装路径，`system-state-provider` 角色仍使用 `enoki-cpu-resource-provider` 二进制与对应 unit/socket 文件名；签名清单中的角色和资源合同以 `system-state-provider/system-state-v2` 为准。
