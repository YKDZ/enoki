...
schema 4 安装尚不具备本机升级专用入口，因此进入 schema 5 的发布转换也必须签名分类为 `replacement-required`。schema 5 起，只有根签名发布转换明确分类为兼容的相邻版本才可由管理员逐台发起升级：非特权探针一次下载并验证完整七角色候选，root 生命周期角色仅从固定暂存目录复验收据并执行同版本事务，不联网，也不接受路径、命令或角色清单。激活前失败保留旧安装；激活开始后的不完整结果进入修复状态且不自动重试。
