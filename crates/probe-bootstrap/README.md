...
schema 4 安装尚不具备本机升级专用入口，因此进入 schema 5 的发布转换也必须签名分类为 `replacement-required`。schema 5 起，只有根签名发布转换明确分类为兼容的相邻版本才可由管理员逐台发起升级：非特权探针一次下载并验证完整七角色候选，root 生命周期角色仅从固定暂存目录复验收据并执行同版本事务，不联网，也不接受路径、命令或角色清单。激活前失败保留旧安装；激活开始后的不完整结果只接受显式 Repair。Repair 由 root journal 签发短期 evidence，清空补充组且禁止重新提权的固定非特权 Acquirer 仅携该 bearer 向绑定 Hub 换取短期 authority，root 离线复验并在独立 durable journal 单次消费后保存完整签名 capsule 与 completion-pending 阶段，再恢复原七角色事务。后续显式 Repair 会先复验磁盘 capsule 并离线续跑，不因短期 authority 已过期或原 Upgrade journal 已推进而重新联网；任何 trust、Identity、receipt 或 postactivation 事实无法闭合时均要求手工重装。
