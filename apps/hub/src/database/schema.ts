...
export const enrollmentTokens = sqliteTable(
  ...
  (table) => [
    ...
    check(
      ...
      sql`(${table.targetKind} = 'new_host' and ${table.targetHostId} is null and ${table.expectedHubOrigin} is null and ${table.expectedProbeId} is null and ${table.expectedProbeVersion} is null and ${table.sourceProbeSha256Json} is null and ${table.targetAssetSetDigest} is null and ${table.targetProbeVersion} is null and ${table.replacementPredecessorEnrollmentId} is null and ${table.replacementPredecessorAssetSetDigest} is null) or (${table.targetKind} = 'existing_host' and ${table.targetHostId} > 0 and ${table.expectedHubOrigin} is null and ${table.expectedProbeId} is null and ${table.expectedProbeVersion} is null and ${table.sourceProbeSha256Json} is null and ${table.targetAssetSetDigest} is null and ${table.targetProbeVersion} is null and ${table.replacementPredecessorEnrollmentId} is null and ${table.replacementPredecessorAssetSetDigest} is null) or (${table.targetKind} = 'manual_reinstall' and ${table.targetHostId} > 0 and length(${table.expectedHubOrigin}) > 0 and length(${table.expectedProbeId}) > 0 and length(${table.expectedProbeVersion}) > 0 and length(${table.sourceProbeSha256Json}) > 0 and length(${table.targetAssetSetDigest}) = 71 and length(${table.targetProbeVersion}) > 0 and ((${table.replacementPredecessorEnrollmentId} is null and ${table.replacementPredecessorAssetSetDigest} is null) or (${table.replacementPredecessorEnrollmentId} != ${table.enrollmentId} and length(${table.replacementPredecessorEnrollmentId}) > 0 and length(${table.replacementPredecessorAssetSetDigest}) = 71))) or (${table.targetKind} is null and ${table.targetHostId} is null and ${table.status} = 'expired' and ${table.replacementPredecessorEnrollmentId} is null and ${table.replacementPredecessorAssetSetDigest} is null)`,
    ),
    check(
